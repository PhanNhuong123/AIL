...
pub fn encode_type_constraint<'ctx>(
    ...
    match builtin {
        ...
        BuiltinSemanticType::PositiveAmount => {
            ...
            let zero = Real::from_real(z3_ctx, 0, 1);
            Ok(vec![v.gt(&zero)])
        }
        ...
        BuiltinSemanticType::Percentage => {
            let v = require_real_or_promote_int(var)?;
            let zero = Real::from_real(z3_ctx, 0, 1);
            let hundred = Real::from_real(z3_ctx, 100, 1);
...
fn require_int<'ctx>(var: &Dynamic<'ctx>) -> Result<Int<'ctx>, EncodeError> {
...
fn require_real_or_promote_int<'ctx>(var: &Dynamic<'ctx>) -> Result<Real<'ctx>, EncodeError> {
